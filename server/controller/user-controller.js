...
const DataURI = await import('data-uri-to-buffer');
dotenv.config();
    ...
    try {
        ...
        if (req.file) {
            ...
            const createImage = async (img) => {
                try {
                    const base64Image = DataURI(img.buffer).toString();
                    const result = await cloudinary.uploader.upload(base64Image, { resource_type: 'image' });
...
// Vercel requires proper configuration for file uploads and environment variables. Make sure to configure Vercel settings accordingly.
